  ...
  can_running_(false)
{
    ...
    sim_timer_ = this->create_wall_timer(
        10ms, std::bind(&VirtualMotorDevice::simulateMotor, this));
    
    publish_timer_ = this->create_wall_timer(
        200ms, std::bind(&VirtualMotorDevice::publishJointState, this)); // 20ms -》 200ms
...
void VirtualMotorDevice::sendTPDO1() {
    ...
    RCLCPP_DEBUG(this->get_logger(), "发送TPDO1: 状态=0x%04X, 位置=%d", status, position);
}
...
void VirtualMotorDevice::sendTPDO2() {
    ...
    RCLCPP_DEBUG(this->get_logger(), "发送TPDO2: 速度=%.1f, 转矩=%.2f", velocity_, torque_);
}
